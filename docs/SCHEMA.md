# SCHEMA.md — InstaFlow AI — Banco de Dados PostgreSQL Completo

## Estratégia de Multi-Tenancy

Toda tabela possui `user_id` referenciando `profiles.id`. O isolamento de dados é garantido por **Row Level Security (RLS)** no nível do banco — nenhum dado de um usuário é acessível por outro, mesmo que a query não filtre explicitamente.

## Row Level Security (RLS)

Cada tabela tem RLS habilitado. As policies usam `auth.uid()` do Supabase Auth para comparar com `user_id`. Isso significa que mesmo que o backend envie uma query sem WHERE, o PostgreSQL filtra automaticamente os dados.

## pgvector para Embeddings

A extensão `pgvector` armazena vetores de até 1536 dimensões (OpenAI text-embedding-3-small). A busca semântica usa `<=>` (distância cosseno) para encontrar chunks relevantes da knowledge base.
...
### Trigger 1: Criar profile ao registrar usuário
...
### Trigger 2: Ativar trial ao criar profile
...
### Trigger 3: Calcular lead score ao criar lead
...
### Trigger 4: Atualizar total_messages ao inserir mensagem
...
### Trigger 5: Atualizar leads_count nas tags
...
### Calcular lead score (recalcular a qualquer momento)
...
### Pipeline stages padrão (inseridos após criação de cada usuário, via Edge Function)
...
Os `access_token` da tabela `instagram_accounts` e `credentials_encrypted` da tabela `integrations` devem ser criptografados usando `pgcrypto`:

```sql
-- Criptografar ao inserir (usando uma chave de app armazenada como secret do Supabase)
UPDATE instagram_accounts
SET access_token = encode(
  encrypt(access_token::bytea, current_setting('app.settings.encryption_key')::bytea, 'aes'),
  'base64'
);

-- A chave de criptografia deve ser configurada como variável de ambiente no Supabase:
-- ALTER DATABASE postgres SET app.settings.encryption_key = 'SUA_CHAVE_SECRETA_32_CHARS';
```

### Realtime

Habilitar Realtime para tabelas que precisam de atualização em tempo real:
...
Para escala, considerar particionar por `created_at`:
...
*Versão: 1.0.0 | Produto: InstaFlow AI | Stack: PostgreSQL 15 + pgvector + Supabase*
